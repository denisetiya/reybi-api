...
impl ProfileService {
    ...
    pub async fn update(db: &PgPool, email: &str, data: UpdateProfileRequest) -> AppResult<User> {
        ...
        .bind(data.name.as_deref().unwrap_or(existing.name.as_deref().unwrap_or("")))
        .bind(data.photo_url.as_deref().or(existing.photo_url.as_deref()))
        .bind(data.role.as_deref().unwrap_or(existing.role.as_deref().unwrap_or("user")))
        .bind(data.phone_number.as_deref().unwrap_or(existing.phone_number.as_deref().unwrap_or("")))

...
pub async fn create(State(state): State<AppState>,
    Locale(locale): Locale, Json(body): Json<CreateReviewRequest>) -> AppResult<Json<serde_json::Value>> {
...
pub async fn update(State(state): State<AppState>,
    Locale(locale): Locale, Path(id): Path<String>, Json(body): Json<UpdateReviewRequest>) -> AppResult<Json<serde_json::Value>> {

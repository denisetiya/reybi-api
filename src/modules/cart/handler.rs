...
pub async fn get(State(state): State<AppState>, Path(user_id): Path<Uuid>, Query(pq): Query<PaginationQuery>) -> AppResult<Json<serde_json::Value>> {
...
pub async fn add(State(state): State<AppState>, Path(user_id): Path<Uuid>, Json(body): Json<AddCartRequest>) -> AppResult<Json<serde_json::Value>> {

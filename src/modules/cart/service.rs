...
impl CartService {
    pub async fn get(db: &PgPool, user_id: Uuid, limit: i64) -> AppResult<Vec<Cart>> {
        sqlx::query_as::<_, Cart>(
            "SELECT * FROM carts WHERE user_id=$1 ORDER BY created_at DESC LIMIT $2"
        ).bind(user_id).bind(limit + 1)
    ...
    pub async fn add(db: &PgPool, user_id: Uuid, data: AddCartRequest) -> AppResult<Cart> {
        ...
        ).bind(user_id).bind(data.product_id).fetch_optional(db).await
            .map_err(|e| AppError::Internal(e.into()))?;

        if let Some(item) = existing {
            let new_qty = item.quantity + data.quantity;
            if new_qty <= 0 {
                sqlx::query("DELETE FROM carts WHERE id=$1").bind(item.id)
            ...
            ).bind(item.id).bind(new_qty).fetch_one(db).await
                .map_err(|e| AppError::Internal(e.into()));
        }
    ...
    pub async fn delete(db: &PgPool, id: Uuid) -> AppResult<()> {
        let r = sqlx::query("DELETE FROM carts WHERE id=$1").bind(id)
            .execute(db).await.map_err(|e| AppError::Internal(e.into()))?;
        if r.rows_affected() == 0 { return Err(AppError::NotFound("Cart item not found".into())); }
        Ok(())
    }
}

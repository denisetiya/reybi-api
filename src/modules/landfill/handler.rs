...
pub async fn create(State(state): State<AppState>,
    Locale(locale): Locale, Json(body): Json<CreateLandfillRequest>) -> AppResult<Json<serde_json::Value>> {
    let item = LandfillService::create(&state.db, body).await?;
    state.cache.invalidate_pattern(keys::landfills_pattern()).await;
    Ok(Json(ok(item, &locale)))
}

pub async fn update(State(state): State<AppState>,
    Locale(locale): Locale, Path(id): Path<String>, Json(body): Json<CreateLandfillRequest>) -> AppResult<Json<serde_json::Value>> {

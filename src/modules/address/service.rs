...
impl AddressService {
    pub async fn create(db: &PgPool, user_id: Uuid, data: CreateAddressRequest) -> AppResult<Address> {
        let id = Uuid::new_v4();
    ...
    pub async fn update(db: &PgPool, user_id: Uuid, data: CreateAddressRequest) -> AppResult<Address> {
        sqlx::query_as::<_, Address>(

...
impl ProfileService {
    ...
    pub async fn update(db: &PgPool, email: &str, data: UpdateProfileRequest) -> AppResult<User> {
        ...
        .bind(email)
        .bind(data.name.as_deref().unwrap_or(&existing.name.unwrap_or_default()))
        .bind(data.photo_url.as_deref().or(existing.photo_url.as_deref()))
        .bind(data.role.as_deref().unwrap_or(&existing.role.unwrap_or_default()))
        .bind(data.phone_number.as_deref().unwrap_or(&existing.phone_number.unwrap_or_default()))
        .fetch_one(db)
        .await
        .map_err(|e| AppError::Internal(e.into()))
    }
